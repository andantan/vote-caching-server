...
export class BallotQueryEventProcessor {
    ...
    public async processBallotQuery(userHash: string): Promise<IBallot[]> {
        logger.debug(`[BallotQueryEventProcessor::processBallotQuery] Attempting to retrieve ballots for UserHash: "${userHash}"`);
        ...
        try {
            ...
            logger.error(`[BallotQueryEventProcessor::processBallotQuery] Database access error during ballot retrieval for UserHash: "${userHash}". Error:`, error);
        ...
        if (ballots === null) {
            logger.warn(`[BallotQueryEventProcessor::processBallotQuery] User with hash "${userHash}" not found.`);
            throw new BallotQueryEventError(BallotQueryEventErrorStatus.USER_NOT_FOUND);
        }

        logger.info(`[BallotQueryEventProcessor::processBallotQuery] Successfully retrieved ${ballots.length} ballots for UserHash: "${userHash}".`);
